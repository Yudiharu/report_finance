...
<?php use App\Models\InvoicearpbmDetail; ?>
...
$grandtotaljumlah = 0;
?>
    <table class="grid1" style="margin-bottom: 25px; width: 100%; font-size: 8px;">
        <thead>
        ...
        <tbody>
            <tr>
                ...
                <?php if ($saldo_awal != null){ ?>
                    ...
                    <td>0</td>
                    <?php if ($bulan == '1') { ?>
                        <td><?php echo number_format($saldo_awal->ending_balance,'2',',','.') ?></td>
                    <?php } else { ?>
                        <td><?php echo number_format($saldo_awal->ending_balance,'2',',','.') ?></td>
                    <?php } ?>
                <?php } else{ ?>
                    ...
                    <td>0</td>
                    <?php if ($bulan == '1') { ?>
                        <td><?php echo number_format($saldo_awal3->beginning_balance,'2',',','.') ?></td>
                    <?php }else { ?>
                        <td><?php echo number_format($saldo_awal2->beginning_balance,'2',',','.') ?></td>
                    <?php } ?>
                <?php } ?>
            </tr>
            <tr>
...
?>
    ...
    <?php } else if (stripos($row->no_transaksi, 'IUM') !== FALSE || stripos($row->no_transaksi, 'DAB') !== FALSE) { ?>
        <?php 
    ...
    <?php } else if (stripos($row->no_transaksi, 'NRJ') !== FALSE) { ?>
        <?php
            $cekdetail = ReturPenjualan::on($konek)->where('no_retur_jual', $row->no_transaksi)->first();
        ?>
            <tr>
                <?php $tanggal2 = $row->tanggal_transaksi; ?>
                <?php if ($tanggal1 == null) { ?>
                ...
                <?php } else { ?>
                    <?php 
                        if ($get_company == '04' || $get_company == '0401'){
                            ...
                            $cek2 = Invoicear::on($konek)->where('no_invoice', $row->no_transaksi)->first();
                            $cust = Customer::where('id', $cek2->kode_customer)->first();
                            $keterangan = $cust->nama_customer;
                        }else if ($get_company == '02' || $get_company == '06') {
                            $cek2 = Invoicearpbm::on($konek)->where('no_invoice', $row->no_transaksi)->first();
                            $cust = Customerpbm::on($konek2)->where('id', $cek2->kode_customer)->first();
                            $keterangan = $cust->nama_customer;
                        }else if ($get_company == '05') {
                            $cek2 = Invoicearsub::on($konek)->where('no_invoice', $row->no_transaksi)->first();
                            $cust = Customersub::where('id', $cek2->kode_customer)->first();
                            $keterangan = $cust->nama_customer;
                        }
                    ?>
